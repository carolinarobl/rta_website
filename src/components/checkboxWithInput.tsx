...
export const CheckboxWithInput = component$(({
    ...
    return <div class={`flex flex-col w-full  shadow-2xl px-4 rounded-2xl ${isSelected.value?"bg-primary-blue text-white":"bg-white text-primary-dark-blue"}`}>
        <div class={`flex items-center justify-start hover:cursor-pointer h-[50px] ${classN}`}>
            <input id={id} type="checkbox" class="peer appearance-none border border-primary-dark-blue w-4 h-4 rounded-full mr-[20px] checked:bg-btn-green checked:border-white" checked={signal.value}/>
        ...
        <textarea rows={3} id={`${id}-textarea`} class="appearance-none border border-primary-dark-blue rounded-2xl text-primary-dark-blue px-1" />
    </div>
});